...
def run_baseline_method(seed, pool=None):
    """
...
def run_sequential_phase1(seed, return_top_n=None, ngen=None, pool=None):
    """
...
# Recebem best_turbine_layout como argumento para evitar problemas com variáveis globais
...
def run_sequential_method(seed, pool=None):
    """
    ...
    best_turbine_layout_ind = run_sequential_phase1(seed, pool=pool)
    
    # Converte o indivíduo para lista de coordenadas (garante que seja uma lista simples)
    # IndividualPhase1 herda de list, então podemos converter diretamente
    if best_turbine_layout_ind is None:
        raise ValueError("run_sequential_phase1 retornou None")
    
    # Converte para lista simples de floats (garante serialização correta para multiprocessing)
    best_turbine_layout = [float(x) for x in best_turbine_layout_ind]
    
    # Verifica se o layout tem o tamanho correto
    if len(best_turbine_layout) != IND_SIZE * 2:
        raise ValueError(f"Layout de turbinas inválido: esperado {IND_SIZE * 2} elementos, recebido {len(best_turbine_layout)}")
...
def run_proposed_method(seed, pool=None):
    """
    ...
    best_layouts_p1 = run_sequential_phase1(seed, return_top_n=N_TOP_LAYOUTS, ngen=NGEN_PROPOSED_P1, pool=pool)
...
if __name__ == "__main__":
    # Cria pool de processos para paralelização
    ...
    try:
        ...
        for run_num in range(1, N_RUNS + 1):
            ...
            pf_baseline, hv_history_baseline = run_baseline_method(SEED, pool=pool)
            ...
            best_sequential, best_turbine_layout = run_sequential_method(SEED, pool=pool)
            ...
            pf_proposed, hv_history_proposed = run_proposed_method(SEED, pool=pool)
        ...
        if len(pf_proposed) > 0:
            ...
            print(f"DEBUG: Execução {run_num}, Métricas Proposed adicionadas. Total all_metrics: {len(all_metrics)}")
        ...
        if N_RUNS > 0:  # Se houve execuções
            ...
            # Ordem fixa dos métodos para garantir consistência
            method_order = ['Baseline', 'Sequential', 'Proposed']
            
            # Cria listas de valores para cada método na ordem correta
            for method in method_order:
                ...
                # Extrai valores para cada métrica (filtra valores válidos)
                hv_vals = [m.get('Hypervolume', 0) for m in method_metrics if 'Hypervolume' in m and m.get('Hypervolume', 0) > 0]
                ...
                # Garante que há pelo menos um valor (necessário para boxplot)
            ...
            # Nota: Quando há apenas 1 valor por método, o boxplot mostra apenas uma linha horizontal (mediana = único valor)
            # Isso é esperado e não é um bug, mas pode ser confuso visualmente
            if len(hv_data) > 0 and len(method_labels) > 0:
